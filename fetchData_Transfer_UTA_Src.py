...
class fetchData_Transfer:
 ...
 for a in range(0,rows,1):
  ...
  try:
       db.src_site_UTA_Src.insert_one(
           {
               "src_site": src_site
           }
       );
          ...
          pass
  try:
       db.dst_cloud_UTA_Src.insert_one(
           {
               "dst_cloud": dst_cloud
           }
  ...
  if (errors_xs != 0):
       if (files_xs != 0):
           ...
           error_prcnt = x * 100
           if((error_prcnt)>70):
            mail.mail("Error in Transfer rate of files between source: "+src_site+" and destination: "+dst_cloud+" is: "+str(round(error_prcnt,2))+"%"+" and the value of error_xs is:"+str(errors_xs)+" and the value of files_xs is:"+str(files_xs))
  try:
     db.Transfer_Tables_UTA_Src.insert_one({
         'src_site':src_site,
         'dst_cloud':dst_cloud,
         'errors_xs':errors_xs,
         'files_xs':files_xs,
